...
from milsymbol.data_generation.data_generation import generate_data_from_milsymbol_app6
from milsymbol.utils.file_manager import copy_and_rename_file, clear_files_in_directory
from milsymbol.utils.image_manager import create_png_equivalents

APP6_ICONS_PATH = os.path.join("..", "APP6-icons")
if not os.path.exists(APP6_ICONS_PATH):
    os.makedirs(APP6_ICONS_PATH)

mapping_file = os.path.join(APP6_ICONS_PATH, "icon-files-mapping.csv")
symbol_code = {}
...
def main():
    ...
    create_png_equivalents(os.path.join(APP6_ICONS_PATH, "svg"), os.path.join(APP6_ICONS_PATH, "png"))

    # update data for symbol selector (csv and png)
    os.makedirs(os.path.join("..", "symbol-selector", "src", "resources"), exist_ok=True)
    shutil.copy(os.path.join(APP6_ICONS_PATH, "icon-files-mapping.csv"),
                os.path.join("..", "symbol-selector", "src", "resources", "icon-files-mapping.csv"))
    shutil.copy(os.path.join(APP6_ICONS_PATH, "icon-files-mapping.csv"),
                os.path.join("..", "..", "MappingCtoD", "dataset", "icon-files-mapping.csv"))
    clear_files_in_directory(os.path.join("..", "symbol-selector", "src", "images", "icons"), [".png", ".svg"])
    put_all_png_in_directory(os.path.join(APP6_ICONS_PATH, "png"),
...
def manage_svg_files(distinct_svg):
    ...
    clear_files_in_directory(os.path.join(APP6_ICONS_PATH), [".png", ".svg"])
...
def create_other_svg_behaviour(file_path_in, folder_out, filename_out):
    ...
    if match:
        ...
        if not os.path.exists(os.path.join(APP6_ICONS_PATH, "svg", folder_out)):
            os.makedirs(os.path.join(APP6_ICONS_PATH, "svg", folder_out))
        ...
        file_path_out = os.path.join(APP6_ICONS_PATH, "svg", folder_out, filename_out + ".svg")
